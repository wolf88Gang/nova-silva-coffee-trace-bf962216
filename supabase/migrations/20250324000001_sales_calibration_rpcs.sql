-- Sales Intelligence — Calibration RPCs
-- Moves P1 analytics from frontend to backend.
-- Requires: v_sales_calibration_dataset, fn_sales_interpret_signal, sales_session_objections
...
-- Uses sales_session_objections for per-type avg_confidence (view has only session-level max)
...
-- ========== GRANTS ==========
GRANT EXECUTE ON FUNCTION public.fn_cal_validation_summary() TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_cal_score_bucket_analysis() TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_cal_objection_analysis() TO authenticated;
