...
export function VitalGate({ children, mode = 'banner' }: VitalGateProps) {
  ...
  // Banner mode: show children + top banner
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 px-4 py-3 rounded-lg border border-border bg-muted/50">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            Los datos mostrados son ilustrativos. Las funciones de escritura (acciones, puntajes) estarán habilitadas en la próxima fase.
          </p>
        </div>
        <Badge variant="secondary" className="shrink-0">Pre-piloto</Badge>
      </div>
      {children}
    </div>
  );
