 ...
 * 5 blocks: Profile, Missing Info, Signals, Objections, Route.
 */
...
function ObjConfidence({ value }: { value: number }) {
  const pct = Math.round(value * 100);
  const levelLabel = pct >= 75 ? 'alta' : pct >= 45 ? 'media' : 'baja';
  return (
    <Badge
      variant="outline"
      className={cn(
...
export function CopilotIntelligencePanel({ profile, interpretation, answeredCount }: Props) {
  const { pain_signals, maturity_level, objections, positioning } = interpretation;
  ...
  return (
    <ScrollArea className="h-full">
      <div className="space-y-3 pr-2 pb-4">
        ...
        <Card className="bg-card border-border">
          ...
          <CardContent className="px-4 pb-3">
            <div className="grid grid-cols-1 gap-1.5 text-xs">
              <ProfileRow label="Tipo de organización" value={label(ORG_TYPE_LABELS, profile.organization_type)} />
              ...
              <ProfileRow label="Presión regulatoria" value={
              ...
              <ProfileRow label="Capacidad presupuestaria" value={label(BUDGET_LABELS, budget)} />
              <ProfileRow label="Urgencia" value={label(TIMELINE_LABELS, timeline)} highlight={timeline === 'immediate'} />
              <ProfileRow label="Organización" value={profile.organization_name ?? '—'} />
            </div>
          </CardContent>
        </Card>

        {/* BLOCK 2: Missing Info */}
          ...
          <Card className="border-amber-500/20">
            <CardHeader className="pb-1.5 pt-3 px-4">
              <CardTitle className="text-[11px] uppercase tracking-wider text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
                <Eye className="h-3.5 w-3.5" /> Información faltante
              </CardTitle>
            </CardHeader>
              ...
              {objections.map((o, i) => (
                <div key={i} className="space-y-1">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <ObjConfidence value={o.confidence} />
                  </div>
                  <p className="text-[10px] text-muted-foreground">
                    Esto sugiere: {o.trigger.replace(/^.*?=\s*/, '').replace(/^Respuesta directa a /, '')}
                  </p>
        ...
        {/* Positioning (subordinate) */}
        {positioning.length > 0 && (
          <div className="px-1 space-y-1">
            <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Posicionamiento</p>
            {positioning.map((p, i) => (
        ...
        {/* Answered count — minimal */}
        {answeredCount > 0 && (
          <p className="text-[10px] text-muted-foreground text-center pt-2">
            {answeredCount} respuestas · {profile.signals.length} señales
          </p>
        )}
      </div>
    </ScrollArea>
