...
import { Users, Clock, Wallet, ClipboardList, Plus, Edit2, MapPin, Calendar, Eye } from 'lucide-react';
import { toast } from 'sonner';

const TARIFA_BASE = 2250; // ₡/hr
...
interface Cuadrilla {
  ...
  zona: string;
  trabajadores: number;
  estado: 'Activa' | 'Asignada' | 'En descanso';
...
export default function JornalesTab() {
  ...
  // Jornal form
  const [jornalForm, setJornalForm] = useState({
    cuadrillaId: '', finca: '', actividad: '', trabajadores: '', horas: '', tarifaHr: String(TARIFA_BASE), observaciones: '',
  });
  ...
  const handleRegistrarJornal = () => {
    if (!jornalForm.cuadrillaId || !jornalForm.finca || !jornalForm.actividad) {
      toast.error('Complete cuadrilla, finca y actividad');
      return;
    }
    ...
    const jornal: RegistroJornal = {
      ...
      cuadrillaId: jornalForm.cuadrillaId,
      finca: jornalForm.finca,
      actividad: jornalForm.actividad,
      trabajadores: t,
      horas: h,
      tarifaHr: Number(jornalForm.tarifaHr) || TARIFA_BASE,
      totalPago: calcTotalPago(),
      observaciones: jornalForm.observaciones,
    };
  ...
  const handleAddCuadrilla = () => {
    ...
    const nueva: Cuadrilla = {
      id: String(Date.now()),
      nombre: cuadrillaForm.nombre,
      capataz: cuadrillaForm.capataz,
      zona: cuadrillaForm.zona || 'Sin asignar',
      trabajadores: Number(cuadrillaForm.trabajadores) || 0,
      estado: 'En descanso',
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setShowNuevaCuadrilla(true)}>
            <Users className="h-4 w-4 mr-1" /> Nueva Cuadrilla
          </Button>
          <Button size="sm" onClick={() => setShowNuevoJornal(true)}>
            <Plus className="h-4 w-4 mr-1" /> Registrar Jornal
          </Button>
        </div>
      </div>
      ...
      <Tabs defaultValue="cuadrillas" className="space-y-4">
                    ...
                    {cuadrillas.map((c) => (
                      <tr key={c.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                        ...
                        <td className="px-4 py-3">
                          <Button variant="ghost" size="sm" onClick={() => setShowEditCuadrilla({ ...c })}>
        ...
        {/* ═══ REGISTROS DE JORNALES ═══ */}
        <TabsContent value="registros">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Registros de Jornales</CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b text-muted-foreground text-left">
                      ...
                      <th className="px-4 py-3 font-medium">Cuadrilla</th>
                      <th className="px-4 py-3 font-medium">Finca</th>
                      <th className="px-4 py-3 font-medium">Actividad</th>
                      <th className="px-4 py-3 font-medium">Trab.</th>
                    ...
                    {jornales.map((j) => {
                      ...
                      return (
                        <tr key={j.id} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                          ...
                          <td className="px-4 py-3">
                            <Button variant="ghost" size="sm" onClick={() => setShowDetalleJornal(j)}>
                ...
                <CardContent className="pt-5 pb-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{c.tipo} | {c.fechaInicio} a {c.fechaFin}</p>
                    </div>
                    {estadoCampanaBadge(c.estado)}
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm mb-3">
                    <div>
                      <span className="text-muted-foreground text-xs">Fincas</span>
                      <p className="font-medium text-foreground">{c.fincas.length}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground text-xs">Cuadrillas</span>
                      <p className="font-medium text-foreground">{c.cuadrillasAsignadas.length}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground text-xs">Avance</span>
                      <p className="font-medium text-foreground">{c.avance}%</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground text-xs">Estado</span>
                      <p className="font-medium text-foreground">{c.estado}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-3">
                    <p className="text-xs text-muted-foreground mb-1">Fincas asignadas:</p>
                    <div className="flex flex-wrap gap-1">
                      {c.fincas.map(f => (
                        <Badge key={f} variant="outline" className="text-xs">
                          <MapPin className="h-3 w-3 mr-1" />{f.split(' - ')[0]}
                        </Badge>
                      ))}
                    </div>
                  </div>
        ...
        <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <ClipboardList className="h-5 w-5 text-primary" /> Registrar Jornal
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Cuadrilla *</Label>
              <Select value={jornalForm.cuadrillaId} onValueChange={v => {
                const cuad = cuadrillas.find(c => c.id === v);
                setJornalForm(s => ({ ...s, cuadrillaId: v, trabajadores: cuad ? String(cuad.trabajadores) : s.trabajadores }));
              }}>
                <SelectTrigger><SelectValue placeholder="Seleccione cuadrilla..." /></SelectTrigger>
                <SelectContent>
                  {cuadrillas.map(c => (
                    <SelectItem key={c.id} value={c.id}>{c.nombre} ({c.trabajadores} trab.)</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select value={jornalForm.finca} onValueChange={v => setJornalForm(s => ({ ...s, finca: v }))}>
                <SelectTrigger><SelectValue placeholder="Seleccione finca..." /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              ...
              <Select value={jornalForm.actividad} onValueChange={v => setJornalForm(s => ({ ...s, actividad: v }))}>
                <SelectTrigger><SelectValue placeholder="Seleccione actividad..." /></SelectTrigger>
                <SelectContent>
                  {ACTIVIDADES.map(a => <SelectItem key={a} value={a}>{a}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="space-y-2">
                ...
                <Input type="number" min="1" value={jornalForm.trabajadores}
                  onChange={e => setJornalForm(s => ({ ...s, trabajadores: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Horas</Label>
                <Input type="number" min="0.5" step="0.5" value={jornalForm.horas}
                  onChange={e => setJornalForm(s => ({ ...s, horas: e.target.value }))} />
              </div>
              <div className="space-y-2">
                <Label>Tarifa (₡/hr)</Label>
                <Input type="number" value={jornalForm.tarifaHr}
                  onChange={e => setJornalForm(s => ({ ...s, tarifaHr: e.target.value }))} />
              </div>
            </div>
            ...
            <div className="p-3 rounded-lg border border-primary/20 bg-primary/5">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Calculo: {jornalForm.trabajadores || 0} trab. x {jornalForm.horas || 0}h x ₡{Number(jornalForm.tarifaHr || 0).toLocaleString()}/hr</span>
              </div>
            ...
            <div className="space-y-2">
              ...
              <Textarea value={jornalForm.observaciones}
                onChange={e => setJornalForm(s => ({ ...s, observaciones: e.target.value }))}
                placeholder="Sector trabajado, notas de rendimiento, etc." rows={2} />
            </div>
      ...
      <Dialog open={showNuevaCuadrilla} onOpenChange={setShowNuevaCuadrilla}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Users className="h-5 w-5 text-primary" /> Nueva Cuadrilla
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div className="space-y-1">
              <Label>Nombre *</Label>
              <Input value={cuadrillaForm.nombre} onChange={e => setCuadrillaForm(s => ({ ...s, nombre: e.target.value }))} placeholder="Ej: Cuadrilla Oeste" />
            </div>
            <div className="space-y-1">
              <Label>Capataz *</Label>
              <Input value={cuadrillaForm.capataz} onChange={e => setCuadrillaForm(s => ({ ...s, capataz: e.target.value }))} placeholder="Nombre completo" />
            </div>
            <div className="space-y-1">
              <Label>Zona asignada</Label>
              <Input value={cuadrillaForm.zona} onChange={e => setCuadrillaForm(s => ({ ...s, zona: e.target.value }))} placeholder="Ej: Vereda Oeste" />
            </div>
            <div className="space-y-1">
              <Label>Trabajadores</Label>
              <Input type="number" min="1" value={cuadrillaForm.trabajadores} onChange={e => setCuadrillaForm(s => ({ ...s, trabajadores: e.target.value }))} />
            </div>
      ...
      <Dialog open={!!showEditCuadrilla} onOpenChange={() => setShowEditCuadrilla(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Edit2 className="h-5 w-5 text-primary" /> Editar Cuadrilla
            </DialogTitle>
          </DialogHeader>
          {showEditCuadrilla && (
            <div className="space-y-3">
              <div className="space-y-1">
                <Label>Nombre</Label>
                <Input value={showEditCuadrilla.nombre} onChange={e => setShowEditCuadrilla(s => s ? { ...s, nombre: e.target.value } : s)} />
              </div>
              <div className="space-y-1">
                <Label>Capataz</Label>
                <Input value={showEditCuadrilla.capataz} onChange={e => setShowEditCuadrilla(s => s ? { ...s, capataz: e.target.value } : s)} />
              </div>
              <div className="space-y-1">
                <Label>Zona</Label>
                <Input value={showEditCuadrilla.zona} onChange={e => setShowEditCuadrilla(s => s ? { ...s, zona: e.target.value } : s)} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label>Trabajadores</Label>
                  <Input type="number" value={showEditCuadrilla.trabajadores} onChange={e => setShowEditCuadrilla(s => s ? { ...s, trabajadores: Number(e.target.value) } : s)} />
                </div>
                <div className="space-y-1">
                  <Label>Estado</Label>
      ...
      <Dialog open={!!showDetalleJornal} onOpenChange={() => setShowDetalleJornal(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Calendar className="h-5 w-5 text-primary" /> Detalle del Jornal
            </DialogTitle>
          </DialogHeader>
          {showDetalleJornal && (
            <div className="space-y-3">
              <div className="text-center p-3">
                <p className="text-3xl font-bold text-foreground">{fmtCRC(showDetalleJornal.totalPago)}</p>
                <p className="text-xs text-muted-foreground mt-1">{showDetalleJornal.fecha}</p>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Cuadrilla</span>
                  <p className="font-medium text-foreground">{cuadrillas.find(c => c.id === showDetalleJornal.cuadrillaId)?.nombre || '-'}</p>
                </div>
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Actividad</span>
                  <p className="font-medium text-foreground">{showDetalleJornal.actividad}</p>
                </div>
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Trabajadores</span>
                  <p className="font-medium text-foreground">{showDetalleJornal.trabajadores}</p>
                </div>
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Horas</span>
                  <p className="font-medium text-foreground">{showDetalleJornal.horas}h</p>
                </div>
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Tarifa</span>
                  <p className="font-medium text-foreground">{fmtCRC(showDetalleJornal.tarifaHr)}/hr</p>
                </div>
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Finca</span>
                  <p className="font-medium text-foreground text-xs">{showDetalleJornal.finca.split(' - ')[0]}</p>
                </div>
              </div>
              {showDetalleJornal.observaciones && (
                <div className="p-2 rounded border border-border">
                  <span className="text-muted-foreground text-xs">Observaciones</span>
                  <p className="text-sm text-foreground">{showDetalleJornal.observaciones}</p>
                </div>
              )}
            </div>
          )}
