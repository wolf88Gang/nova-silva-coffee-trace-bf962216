...
import { Boxes, AlertTriangle, Wallet, Plus, Minus, Edit2, PackagePlus, History, MapPin, Wrench, TrendingUp } from 'lucide-react';
...
interface Insumo {
  id: string; producto: string; cat: string; stock: number; minimo: number; unidad: string; costoUnitario: number;
}

interface Movimiento {
  id: string; insumoId: string; tipo: 'entrada' | 'salida'; cantidad: number; fecha: string; motivo: string; destino: string; observaciones: string;
}

interface SugerenciaSalida {
  motivo: string; destino: string; cantidadSugerida: number; fuente: string;
}

const MOTIVOS_SALIDA = ['Aplicacion programada', 'Recomendacion Nova Guard', 'Recomendacion VITAL', 'Entrega a productor', 'Despacho a finca', 'Mantenimiento de equipo', 'Merma / Vencimiento', 'Otro'];
const MOTIVOS_ENTRADA = ['Compra a proveedor', 'Donacion', 'Devolucion de productor', 'Transferencia entre bodegas', 'Otro'];
const FINCAS_DESTINO = ['Finca El Progreso - Vereda Norte', 'Finca La Union - Vereda Sur', 'Finca San Jose - Vereda Central', 'Finca Las Flores - Vereda Este', 'Bodega Central', 'Entrega directa a productor'];
...
const valorPorCategoria = [
  { cat: 'Fertilizantes', valor: 1282500 },
  { cat: 'Empaque', valor: 360000 },
  { cat: 'Herramientas', valor: 225000 },
  { cat: 'Fungicidas', valor: 98400 },
  { cat: 'Enmiendas', valor: 76000 },
  { cat: 'Biocontrol', valor: 45000 },
];

const stockStatus = [
  { name: 'OK', value: 3, color: 'hsl(142, 60%, 45%)' },
  { name: 'Bajo', value: 2, color: 'hsl(45, 90%, 50%)' },
  { name: 'Crítico', value: 1, color: 'hsl(0, 65%, 50%)' },
];

const movimientosMensual = [
  { mes: 'Nov', entradas: 12, salidas: 8 },
  { mes: 'Dic', entradas: 5, salidas: 10 },
  { mes: 'Ene', entradas: 15, salidas: 12 },
  { mes: 'Feb', entradas: 8, salidas: 14 },
];
...
function getSugerenciasSalida(insumo: Insumo): SugerenciaSalida[] {
  ...
  if (insumo.cat === 'Fertilizantes') {
    sugerencias.push({ motivo: 'Aplicacion programada', destino: 'Finca El Progreso - Vereda Norte', cantidadSugerida: 5, fuente: 'Plan de fertilizacion Feb-Mar: 5 sacos para 2.5 ha' });
    sugerencias.push({ motivo: 'Entrega a productor', destino: 'Entrega directa a productor', cantidadSugerida: 3, fuente: 'Credito insumos activo: Juan Perez Lopez (3 sacos aprobados)' });
  }
  if (insumo.cat === 'Fungicidas') sugerencias.push({ motivo: 'Recomendacion Nova Guard', destino: 'Finca La Union - Vereda Sur', cantidadSugerida: 4, fuente: 'Alerta Roya: Incidencia 12% en Vereda Sur' });
  if (insumo.cat === 'Biocontrol') sugerencias.push({ motivo: 'Recomendacion Nova Guard', destino: 'Finca San Jose - Vereda Central', cantidadSugerida: 2, fuente: 'Alerta Broca: Nivel 3% en Vereda Central' });
  if (insumo.cat === 'Enmiendas') sugerencias.push({ motivo: 'Recomendacion VITAL', destino: 'Finca Las Flores - Vereda Este', cantidadSugerida: 4, fuente: 'VITAL: pH bajo detectado, encalado sugerido' });
  return sugerencias;
}

const getEstado = (stock: number, minimo: number) => { if (stock <= minimo * 0.3) return 'critico'; if (stock < minimo) return 'bajo'; return 'ok'; };
const estadoBadge = (e: string) => { if (e === 'ok') return <Badge className="bg-emerald-600 text-white border-0">OK</Badge>; if (e === 'bajo') return <Badge className="bg-amber-500 text-white border-0">Bajo</Badge>; return <Badge variant="destructive">Critico</Badge>; };
...
export default function InventarioTab() {
  ...
  const handleMovimiento = () => {
    ...
    const mov: Movimiento = { id: String(Date.now()), insumoId: showMovimiento.insumo.id, tipo: showMovimiento.tipo, cantidad: qty, fecha: new Date().toISOString().slice(0, 10), motivo, destino: showMovimiento.tipo === 'salida' ? destino : motivo, observaciones };
    setMovimientos(prev => [mov, ...prev]);
    setInsumos(prev => prev.map(i => { if (i.id !== showMovimiento.insumo.id) return i; const newStock = showMovimiento.tipo === 'entrada' ? i.stock + qty : Math.max(0, i.stock - qty); return { ...i, stock: newStock }; }));
  ...
  const handleAddInsumo = () => {
    ...
    const nuevo: Insumo = { id: String(Date.now()), producto: nuevoInsumo.producto, cat: nuevoInsumo.cat, stock: Number(nuevoInsumo.stock) || 0, minimo: Number(nuevoInsumo.minimo) || 0, unidad: nuevoInsumo.unidad, costoUnitario: Number(nuevoInsumo.costoUnitario) || 0 };
    setInsumos(prev => [...prev, nuevo]);
    toast.success(`Insumo \\"${nuevo.producto}\\" agregado`);
    setShowAdd(false); setNuevoInsumo({ producto: '', cat: 'Fertilizantes', stock: '', minimo: '', unidad: '', costoUnitario: '' });
  };
  ...
  const kpis = [
    ...
    { label: 'Equipos Operativos', value: String(insumos.filter(i => i.cat === 'Herramientas').length), icon: Wrench, color: '' },
  ];

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-sm text-muted-foreground">Gestiona insumos, equipos, vehículos y maquinaria de la organización</p>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setShowHistorial(true)}><History className="h-4 w-4 mr-1" /> Historial ({movimientos.length})</Button>
          <Button size="sm" onClick={() => setShowAdd(true)}><PackagePlus className="h-4 w-4 mr-1" /> Agregar Insumo</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {kpis.map((k) => (
          <Card key={k.label}><CardContent className="pt-4 pb-3">
            <div className="flex items-center gap-2 mb-1"><k.icon className={`h-4 w-4 ${k.color || 'text-primary'}`} /><span className="text-xs text-muted-foreground">{k.label}</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={valorPorCategoria} layout="vertical">
                ...
                <YAxis type="category" dataKey="cat" tick={{ fontSize: 10, fill: 'hsl(var(--muted-foreground))' }} width={80} />
                ...
                <Bar dataKey="valor" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]} name="Valor" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Estado de Stock</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={stockStatus} cx="50%" cy="50%" innerRadius={40} outerRadius={70} dataKey="value" label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
                  {stockStatus.map((d, i) => <Cell key={i} fill={d.color} />)}
                </Pie>
        ...
        <Card>
          <CardHeader className="pb-2"><CardTitle className="text-base">Movimientos Mensuales</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={movimientosMensual}>
                ...
                <Bar dataKey="entradas" fill="hsl(142, 60%, 45%)" radius={[4, 4, 0, 0]} name="Entradas" />
                <Bar dataKey="salidas" fill="hsl(0, 65%, 50%)" radius={[4, 4, 0, 0]} name="Salidas" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      {/* Insumos table */}
      <Card>
        <CardHeader className="pb-2"><CardTitle className="text-base">Inventario de Insumos</CardTitle></CardHeader>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead><tr className="border-b text-muted-foreground text-left">
                <th className="px-4 py-3 font-medium">Insumo</th><th className="px-4 py-3 font-medium">Categoria</th>
                <th className="px-4 py-3 font-medium">Stock</th><th className="px-4 py-3 font-medium">Minimo</th>
                <th className="px-4 py-3 font-medium">Unidad</th><th className="px-4 py-3 font-medium">Costo unit.</th>
                <th className="px-4 py-3 font-medium">Estado</th><th className="px-4 py-3 font-medium">Acciones</th>
              </tr></thead>
              <tbody>
                {insumos.map((item) => {
                    ...
                    <tr key={item.id} className={`border-b last:border-0 hover:bg-muted/50 transition-colors ${estado !== 'ok' ? 'bg-amber-50/50 dark:bg-amber-900/10' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" title="Entrada" onClick={() => { setShowMovimiento({ insumo: item, tipo: 'entrada' }); resetMovForm(); }}><Plus className="h-3.5 w-3.5 text-emerald-600" /></Button>
                          <Button variant="ghost" size="sm" title="Salida" onClick={() => { setShowMovimiento({ insumo: item, tipo: 'salida' }); resetMovForm(); }}><Minus className="h-3.5 w-3.5 text-destructive" /></Button>
                          <Button variant="ghost" size="sm" title="Editar" onClick={() => setShowEdit({ ...item })}><Edit2 className="h-3.5 w-3.5" /></Button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {showMovimiento?.tipo === 'entrada' ? <><Plus className="h-5 w-5 text-emerald-600" /> Registrar Entrada</> : <><Minus className="h-5 w-5 text-destructive" /> Registrar Salida</>}
            </DialogTitle>
          </DialogHeader>
          {showMovimiento && (
            <div className="space-y-4">
              ...
              {showMovimiento.tipo === 'salida' && sugerencias.length > 0 && (
                <div className="space-y-2">
                  ...
                  {sugerencias.map((s, i) => (
                    <button key={i} onClick={() => applySugerencia(s)} className="w-full text-left p-3 rounded-lg border border-primary/20 bg-primary/5 hover:border-primary/50 hover:bg-primary/10 transition-all">
                      <div className="flex items-start gap-2">
                        <MapPin className="h-4 w-4 text-primary mt-0.5 shrink-0" />
                        <div><p className="text-sm font-medium text-foreground">{s.cantidadSugerida} {showMovimiento.insumo.unidad} → {s.destino}</p><p className="text-xs text-muted-foreground mt-0.5">{s.fuente}</p></div>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      <Dialog open={showHistorial} onOpenChange={setShowHistorial}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader><DialogTitle className="flex items-center gap-2"><History className="h-5 w-5 text-primary" /> Historial de Movimientos</DialogTitle></DialogHeader>
                  ...
                  <div key={m.id} className="p-3 rounded-lg border border-border">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        {m.tipo === 'entrada' ? <Badge className="bg-emerald-600 text-white border-0">Entrada</Badge> : <Badge variant="destructive">Salida</Badge>}
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1"><Label>Stock minimo</Label><Input type="number" value={nuevoInsumo.minimo} onChange={e => setNuevoInsumo(s => ({ ...s, minimo: e.target.value }))} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
        ...
        <DialogContent className="max-w-md">
          ...
          {showEdit && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1"><Label>Stock minimo</Label><Input type="number" value={showEdit.minimo} onChange={e => setShowEdit(s => s ? { ...s, minimo: Number(e.target.value) } : s)} /></div>
